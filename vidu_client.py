...
class ViduAudioModel(str, Enum):
    """音频模型枚举"""
    VIDU_AUDIO = "vidu-audio"
    VIDU_AUDIO_PRO = "vidu-audio-pro"
...
class ViduClient:
    ...
    MODEL_DURATION_LIMITS = {
        ...
        ViduModel.VIDU2_0: [4],  # vidu2.0 只支持4秒
    ...
    def _validate_duration(self, model: Union[str, ViduModel], duration: Optional[int]) -> int:
        ...
        # 检查时长是否在允许范围内
        allowed_durations = self.MODEL_DURATION_LIMITS[model_enum]
        if duration not in allowed_durations:
            raise ValueError(
                f"模型 {model_str} 不支持 {duration} 秒时长。"
    ...
    def upscale_pro(
        ...
        return self._make_request('POST', '/ent/v2/upscale-pro', request_data)
    ...
    def lip_sync(
        ...
        if not audio_url:
            raise ValueError("audio_url参数为必填项")
    ...
    def text_to_audio(
        ...
        Args:
            model: 音频模型名称 (vidu-audio, vidu-audio-pro)
            text: 文本内容（必填）
            voice_id: 声音ID（可选）
            payload: 透传参数（可选，最大1048576字符）
            callback_url: 回调URL（可选）
